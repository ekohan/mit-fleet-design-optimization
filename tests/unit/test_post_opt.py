...
def test_no_merges(monkeypatch):
    ...
    import fleetmix.post_optimization.core
    original_gen = fleetmix.post_optimization.core.generate_post_optimization_merges
    monkeypatch.setattr(fleetmix.post_optimization.core, "generate_post_optimization_merges", fake_gen)
    ...
    try:
        ...
        result = post_opt.improve_solution(initial_solution, make_config_df(), pd.DataFrame(), params)
        assert result is initial_solution
        assert calls['gen'] == 1
        assert calls['solve'] == 0
    finally:
        # Restore the original functions to avoid affecting other tests
        fleetmix.post_optimization.core.generate_post_optimization_merges = original_gen
